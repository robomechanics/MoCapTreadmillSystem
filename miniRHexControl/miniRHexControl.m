function varargout = miniRHexControl(varargin)
% MINIRHEXCONTROL MATLAB code for miniRHexControl.fig
%      MINIRHEXCONTROL, by itself, creates a new MINIRHEXCONTROL or raises the existing
%      singleton*.
%
%      H = MINIRHEXCONTROL returns the handle to a new MINIRHEXCONTROL or the handle to
%      the existing singleton*.
%
%      MINIRHEXCONTROL('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MINIRHEXCONTROL.M with the given input arguments.
%
%      MINIRHEXCONTROL('Property','Value',...) creates a new MINIRHEXCONTROL or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before miniRHexControl_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to miniRHexControl_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help miniRHexControl

% Last Modified by GUIDE v2.5 08-Aug-2018 10:17:10

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @miniRHexControl_OpeningFcn, ...
                   'gui_OutputFcn',  @miniRHexControl_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before miniRHexControl is made visible.
function miniRHexControl_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to miniRHexControl (see VARARGIN)

% USER INPUTS
% set to false to run gui without Optitrack Connection, for debugging only
handles.optiDataConnection = true;

% Setup Bluetooth connection to robot
handles.btObj = Bluetooth('Robotis BT-210',1);
fopen(handles.btObj); % Open the bluetooth object
disp('miniRHex Bluetooth Connected')
...
%Create timer - NOTE miniRhex starts missing packets at 30Hz command update
handles.t_update = timer('TimerFcn',{@updateData,hObject}, 'Period',1/25,'ExecutionMode','fixedRate','BusyMode','queue');

% Choose default command line output for miniRHexControl
...
% UIWAIT makes miniRHexControl wait for user response (see UIRESUME)
...
if handles.mode == 3 || handles.mode == 4
    ...
    gaitParams = [handles.P1 handles.P2 handles.P3 handles.P4 handles.P5 handles.P6];

    RHexControl(handles.btObj, handles.robotOn, angVel, gaitParams);
end
...
x0 = [handles.P1 handles.P2 handles.P3 handles.P4 handles.P5 handles.P6];

% create anonymous function for x
costFunc = @(x)costFunction_miniRHex(x,hObject);
...
function varargout = miniRHexControl_OutputFcn(hObject, eventdata, handles) 
...
case 'Gait Optimization'
    ...
    handles.optData.gait = [0 0 0 0 0 0];
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
fclose(handles.btObj);
disp('miniRHex Bluetooth Disconnected')
